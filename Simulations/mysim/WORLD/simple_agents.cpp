...
void SIMPLE_Agents::add_noise( vector <double> &readings ){
  ...
  for(int i = 0; i < readings.size();/*num_IR_sensors;*/ i++){
    if( readings[i] == -1.0 ){
      ...
      readings[i] = ((double)(gsl_rng_uniform_int(GSL_randon_generator::r_rand, 150)));
...
void SIMPLE_Agents::get_randb_reading( vector <double> _nest_pos, vector <double> &_reading){
  randb_from = btVector3(0.0,0.0,0.0);
  randb_to   = btVector3(0.0,0.0,0.0);
  this->pos = this->get_pos();
  btMatrix3x3 m = btMatrix3x3(body->getWorldTransform().getRotation());
...
/*                                  GYRO READING                           */
/* ----------------------------------------------------------------------- */
void SIMPLE_Agents::get_gyroY_readings( vector <double> &readings ){
  double current_rot = PI + this->get_rotation();   
  double prev_rot = this->previous_rot[1];
  if( cos( current_rot ) > 0.0  ){
    if( sin ( prev_rot ) < 0.0  && sin( current_rot ) >= 0.0 ){
      readings[0] = current_rot + ( TWO_PI - prev_rot);
    }
    else if( sin ( prev_rot ) > 0.0  && sin( current_rot ) <= 0.0 ){
      readings[0] = -( (TWO_PI - current_rot) + prev_rot );
    }
    else{
      readings[0] = current_rot - prev_rot;
    }
  }
  else{
    readings[0] = current_rot - prev_rot;
  ...
  readings[0] += gsl_rng_uniform_pos( GSL_randon_generator::r_rand )*0.02 - 0.01;


  if( readings[0] > 1.0 ) readings[0] = 1.0;
  else if ( readings[0] < -1.0 ) readings[0] = -1.0;

  /* cerr << " \n cur_rot = " << current_rot
       << " \n pre_rot = " << prev_rot
       << " \n cos( cur ) = " << cos( current_rot )
       << " \n sin( cur ) = " << sin( current_rot )
       << " \n sin( prev_rot ) = " << sin( prev_rot )
       << " \n gyro read = " << readings[0]
       << endl; */
    
  this->previous_rot[1] = current_rot;
}
